...
class AtCoderProblem(onlinejudge.problem.Problem):
    ...
    def _find_sample_tags(self, soup):
        ...
        for pre in soup.find_all("pre"):
            ...
            if (
                prv and prv.name == "h3" and prv.string
            ):  # AtCoder's javascript recognizes `h3+pre' as a sample input/output
                result += [(pre, prv)]
            else:
                if (
                    pre.parent and pre.parent.name == "section"
                ):  # AtCoder's javascript sometimes fails. e.g. http://abc001.contest.atcoder.jp/tasks/abc001_1
    ...
    def get_url(self):
        return "http://{}.contest.atcoder.jp/tasks/{}".format(
                ...
                for _ in range(3):
                    ...
                    if tag.name in ("pre", "blockquote"):
                        ...
                        for it in tag:
                            s += (
                                it.string or it
                            )  # AtCoder uses <var>...</var> for math symbols
                        return s
...
class AtCoderSubmission(onlinejudge.submission.Submission):
    ...
    def get_url(self):
        return "http://{}.contest.atcoder.jp/submissions/{}".format(
