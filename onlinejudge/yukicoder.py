...
class YukicoderService(onlinejudge.service.Service):
    ...
    def _issue_official_api(self, api, id=None, name=None, session=None):
        ...
        if id is not None:
            ...
            url = "https://yukicoder.me/api/v1/{}/name/{}".format(
    ...
    # NOTE: loginしてると
    def get_problems(
        self, page, comp_problem=True, other=False, sort=None, session=None
    ):
        assert isinstance(page, int) and page >= 1
    ...
    def _get_and_parse_the_table(self, url, session=None):
        ...
        columns = [
            th.text.strip() for th in table.find("thead").find("tr") if th.name == "th"
        ]
        data = []
