...
class CodeforcesService(onlinejudge.service.Service):
    ...
    def from_url(cls, s):
        ...
        if result.scheme in ("", "http", "https") and result.netloc == "codeforces.com":
            return cls()
...
class CodeforcesProblem(onlinejudge.problem.Problem):
    def __init__(self, contest_id, index, kind=None):
        ...
        self.kind = (
            kind
        )  # It seems 'gym' is specialized, 'contest' and 'problemset' are the same thing
        if kind is None:
    ...
    def download(self, session=None):
        ...
        for tag in soup.find_all(
            "div", class_=re.compile("^(in|out)put$")
        ):  # Codeforces writes very nice HTML :)
    ...
    def from_url(cls, s):
        ...
        if result.scheme in ("", "http", "https") and result.netloc == "codeforces.com":
            ...
            table[
                ...
                r"^/contest/([0-9]+)/problem/([0A-Za-z])$"
            )  # example: http://codeforces.com/contest/538/problem/H
            table[
                "problemset"
            ] = (
                r"^/problemset/problem/([0-9]+)/([0A-Za-z])$"
            )  # example: http://codeforces.com/problemset/problem/700/B
            table[
                "gym"
            ] = (
                r"^/gym/([0-9]+)/problem/([0A-Za-z])$"
            )  # example: http://codeforces.com/gym/101021/problem/A
