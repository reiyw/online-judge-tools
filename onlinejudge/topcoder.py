...
class TopCoderLongContestProblem(onlinejudge.problem.Problem):
    ...
    def get_url(self):
        return "https://community.topcoder.com/tc?module=MatchDetails&rd=" + str(
            self.rd
        )
    ...
    def submit(self, code, language, kind="example", session=None):
        ...
        path = [
            tag.attrs["href"]
            for tag in soup.find_all("a", text="Submit")
            if ("rd=%d" % self.rd) in tag.attrs["href"]
        ]
        if len(path) == 0:
            log.error(
                "link to submit not found:  Are you logged in?  Are you registered?  Is the contest running?"
            )
            return None
        assert len(path) == 1
        path = path[0]
        ...
        if "module=SubmitSuccess" in resp.content.decode(resp.encoding):
            url = "http://community.topcoder.com/longcontest/?module=SubmitSuccess&rd={}&cd={}&compid={}".format(
                self.rd, self.cd, self.compid
            )
            ...
            soup = bs4.BeautifulSoup(
                resp.content.decode(resp.encoding), utils.html_parser
            )
    ...
    def get_standings(self, session=None):
        ...
        for start in itertools.count(1, 100):
            # get
            url = "https://community.topcoder.com/longcontest/?sc=&sd=&nr=100&sr={}&rd={}&module=ViewStandings".format(
                start, self.rd
            )
            resp = utils.request("GET", url, session=session)
