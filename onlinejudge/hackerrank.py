...
class HackerRankService(onlinejudge.service.Service):
    def login(self, get_credentials, session=None):
        ...
        resp = form.request(
            session, action="/auth/login", headers={"X-CSRF-Token": csrftoken}
        )
...
class HackerRankProblem(onlinejudge.problem.Problem):
    ...
    def download_with_running_code(self, session=None):
        ...
        url = "https://www.hackerrank.com/rest/contests/{}/challenges/{}/compile_tests/{}?_={}".format(
            self.contest_slug, self.challenge_slug, it["model"]["id"], unixtime
        )
        # sleep
        log.status("sleep(3)")
        time.sleep(3)
        # get
        resp = utils.request(
            "GET", url, session=session, headers={"X-CSRF-Token": csrftoken}
        )
        # parse
        it = json.loads(resp.content.decode())
        log.debug("json: %s", it)
    ...
    def download_with_parsing_html(self, session=None):
        session = session or utils.new_default_session()
        url = "https://www.hackerrank.com/rest/contests/{}/challenges/{}".format(
            self.contest_slug, self.challenge_slug
        )
    ...
    def get_url(self):
        if self.contest_slug == "master":
            return "https://www.hackerrank.com/challenges/{}".format(
                self.challenge_slug
            )
        else:
            return "https://www.hackerrank.com/contests/{}/challenges/{}".format(
                self.contest_slug, self.challenge_slug
            )
