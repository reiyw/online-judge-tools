...
class AOJProblem(onlinejudge.problem.Problem):
    ...
    def download_samples(self, session=None):
        ...
        for pre in soup.find_all("pre"):
            ...
            if (
                hn
                and hn.name in ["h2", "h3"]
                and hn.string
                and any(filter(lambda keyword: keyword in hn.string.lower(), keywords))
            ):
    ...
    def download_system(self, session=None):
        session = session or utils.new_default_session()
        get_url = lambda case, type: "http://analytic.u-aizu.ac.jp:8080/aoj/testcase.jsp?id={}&case={}&type={}".format(
            self.problem_id, case, type
        )
        testcases = []
        for case in itertools.count(1):
            ...
            # get
            resp = utils.request(
                "GET", get_url(case, "in"), session=session, raise_for_status=False
            )
    ...
    def get_url(self):
        return "http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id={}".format(
            self.problem_id
        )
