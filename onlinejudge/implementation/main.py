...
from onlinejudge.implementation.command.split_input import (
    split_input,
...
""",
    ...
    subparser.add_argument(
        "-f",
        "--format",
        help='a format string to specify paths of cases (defaut: "sample-%%i.%%e" if not --system)',
    ...
    subparser.add_argument(
        "-n", "--dry-run", action="store_true", help="don't write to files"
    )
    subparser.add_argument(
        "-a", "--system", action="store_true", help="download system testcases"
    )
...
""",
    ...
    subparser.add_argument(
        "-l", "--language", help="narrow down language choices if ambiguous"
    )
    ...
    subparser.add_argument(
        "--no-guess-latest", action="store_false", dest="guess_cxx_latest"
    )
    subparser.add_argument(
        "--guess-cxx-latest",
        action="store_true",
    ...
    subparser.add_argument(
        ...
        default="gcc",
        help="use the specified C++ compiler if both of GCC and Clang are available (default: gcc)",
    )
    subparser.add_argument(
        "--guess-python-version",
        choices=("2", "3", "auto", "all"),
        default="auto",
        help='shebang or modelines are used by default. write something like "#!/usr/bin/env python3". (default: auto)',
    )
    subparser.add_argument(
        "--guess-python-interpreter",
        choices=("cpython", "pypy", "all"),
        default="cpython",
        help="use the specified Python interpreter if both of CPython and PyPy are available (default: cpython)",
    )
    subparser.add_argument(
        "--format-dos2unix",
        action="store_true",
    ...
    subparser.add_argument(
        "--full-submission",
        action="store_true",
        help='for TopCoder Marathon Match. use this to do "Submit", the default behavier is "Test Examples".',
    )
...
""",
    ...
    subparser.add_argument(
        ...
        "--format",
        default="%s.%e",
        help='a format string to recognize the relationship of test cases. (default: "%%s.%%e")',
    )
    subparser.add_argument(
        "-d",
        "--directory",
    ...
    subparser.add_argument(
        "--rstrip", action="store_true", help="rstrip output before compare (default)"
    )
    subparser.add_argument(
        "-s",
        "--silent",
        action="store_true",
        help="don't report output and correct answer even if not AC  (for --mode all)",
    )
    subparser.add_argument(
        "-e",
        "--error",
        type=float,
        help="check as floating point number: correct if its absolute or relative error doesn't exceed it",
    )
    subparser.add_argument("-t", "--tle", type=float)
    subparser.add_argument(
        "-i", "--print-input", action="store_true", help="print input cases if not AC"
    )
    subparser.add_argument(
        "--no-ignore-backup", action="store_false", dest="ignore_backup"
    )
    subparser.add_argument(
        "--ignore-backup",
        action="store_true",
        help='ignore backup files and hidden files (i.e. files like "*~", "\\#*\\#" and ".*") (default)',
    )
    subparser.add_argument(
        "test", nargs="*", help="paths of test cases. (if empty: globbed from --format)"
    )
...
""",
    ...
    subparser.add_argument(
        "--scanf", action="store_true", help="use scanf instead of cin"
    )
...
""",
    ...
    subparser.add_argument(
        "-i", "--input", metavar="PATH", required=True, help="input file  (required)"
    )
    subparser.add_argument(
        "-o",
        "--output",
