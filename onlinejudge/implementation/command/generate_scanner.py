...
def simplify_expr(s):
    ...
    return str(
        sympy_parser.parse_expr(
            s, local_dict=local_dict, transformations=transformations
        )
    )
    ...
    def go(it):
        ...
        while i < len(it):
            if i - 1 >= 0 and it[i - 1]["kind"] == "read" and it[i]["kind"] == "decl":
                ...
                i - 1 >= 0
                and it[i - 1]["kind"] == it[i]["kind"]
                and it[i]["kind"] in ["decl", "decl-vector", "read", "read-indexed"]
            ):
...
def export(it, repeat_macro=None, use_scanf=False):
    def go(it, nest):
        if it["kind"] == "decl":
            ...
            if it["targets"]:
                return "vector<int> {}; ".format(
                    ", ".join(
                        map(
                            lambda x: x["name"] + paren_if(x["length"], "()"),
                            it["targets"],
                        )
                    )
                )
            ...
            if it["kind"] == "read":
                ...
                items = list(
                    map(
                        lambda x: x["name"] + "[" + "ijk"[nest - x["index"] - 1] + "]",
                        it["targets"],
                    )
                )
            if use_scanf:
                return 'scanf("{}", {});\n'.format(
                    "%d" * len(items), ", ".join(map(lambda s: "&" + s, items))
                )
            else:
                return "cin >> {};\n".format(" >> ".join(items))
        elif it["kind"] == "loop":
