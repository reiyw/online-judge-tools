    ...
    with utils.with_cookiejar(utils.new_default_session(), path=args.cookie) as sess:
        ...
        if args.guess:
            ...
            if args.language is not None:
                matched_lang_ids = select_ids_of_matched_languages(
                    args.language.split(), matched_lang_ids, language_dict=langs
                )
        else:
        ...
        if problem.get_service().get_name() == "topcoder":
            if args.full_submission:
                ...
                kwargs["kind"] = "example"
        submission = problem.submit(
            code, language=args.language, session=sess, **kwargs
        )
...
def select_ids_of_matched_languages(
    words, lang_ids, language_dict, split=False, remove=False
):
...
def guess_lang_ids_of_file(
    ...
    select = lambda word, lang_ids, **kwargs: select_ids_of_matched_languages(
        [word], lang_ids, **kwargs, language_dict=language_dict
    )
    ...
    if ext in ("cpp", "cxx", "cc", "C"):
        ...
        if cxx_latest:
            ...
            for compiler in (None, "gcc", "clang"):  # use the latest for each compiler
                ...
                for lang_id in ids:
                    m = re.search(
                        r"c\+\+\w\w", language_dict[lang_id]["description"].lower()
                    )
                    if m:
        ...
        for data in table:
            if ext in data["exts"]:
                for name in data["names"]:
                    lang_ids += select(
                        name, language_dict.keys(), split=data.get("split", False)
                    )
